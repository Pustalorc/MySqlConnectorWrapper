...
{
    private readonly ConnectorWrapper<DefaultConnectorConfiguration, string> m_Database =
    ...
    public void CreateDropTableTest()
    {
        ...
        output = m_Database.ExecuteQuery(
    ...
    {
        m_Database.ExecuteQuery(m_Database.CreateQuery("CREATE TABLE",
            "CREATE TABLE `test` (`id` VARCHAR(5) NOT NULL);", EQueryType.NonQuery));

        var output =
            m_Database.ExecuteQuery(
                m_Database.CreateQuery("SHOW TABLES", "SHOW TABLES LIKE 'test';", EQueryType.Scalar));
    ...
    public void ExecuteTransactionTest()
    {
        ...
        var allOut = m_Database.ExecuteTransaction(
            m_Database.CreateQuery("SHOW TABLE", "SHOW TABLES LIKE 'test';", EQueryType.Scalar),
    ...
    {
        var output = m_Database.ExecuteQuery(m_Database.CreateQuery("SHOW TABLE", "SHOW TABLES LIKE 'test';",
            EQueryType.Scalar, callbacks: new Query<string>.QueryCallback[]
            {
                o =>
                {
    ...
    public void MultiThreadTest()
    {
        ...
        autoReset2.WaitOne();
        m_Database.ExecuteTransaction(
            m_Database.CreateQuery("DROP TABLE", "DROP TABLE IF EXISTS `test`;", EQueryType.NonQuery),
